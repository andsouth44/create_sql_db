...
file_handler = logging.FileHandler('{}/conf.log'.format(cwd), mode='w')
...
def populate_tables():
    """
    Function to loop through a series of Juniper devices based on input list.
    1. Create new MySQL DB
    2. Connect to each device
    3. Download the configuration file in XML format
    4. Look through appropriate sections of the file and save parameters to a
    MySQL database
    """
    start = datetime.now()

    """ Delete any existing DB and create new DB with new tables"""
    create_tables()
    ...
    with open(cwd + '/switchlist.txt') as infile:
        for hostname in infile:
            ...
            if config.find('routing-instances') is not None:
                ...
                for item in conf_ris:
                    ...
                    try:
                        cur.execute(
                            'insert into routing_instances (instance_name, type, rd, rt, protocol) values (%s, %s, %s, %s, %s)',
                            (ri_details['instance_name'], ri_details['type'], ri_details['rd'],
                             ri_details['rt'], ri_details['protocol']))
                    except Exception as e:
                        logger.warning('Error: {}'.format(e))
            ...
            if config.find('class-of-service/scheduler-maps') is not None:
                ...
                for scheduler_map in scheduler_maps:
                    ...
                    try:
                        cur.execute('insert into scheduler_maps (name) values (%s)', (scheduler_map_details['name'],))
                    except Exception as e:
                        logger.warning('Error: {}'.format(e))
                        ...
                        for forwarding_class in scheduler_map.iter('forwarding-class'):
                            ...
                            for k in scheduler_map_details:
                                if k[:16] in ('forwarding_class',):
                                    try:
                                        cur.execute('update scheduler_maps set {} = %s where name = %s'.format(k, ),
